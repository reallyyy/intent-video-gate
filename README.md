...
- **Bilibili stacked subtitles** -- Bilibili videos are eligible when the API reports usable English, bilingual, or short Chinese AI subtitle tracks. Chinese-only subtitle tracks are translated server-side, validated against the current source subtitle file, and rendered as a custom Chinese + English overlay on Bilibili watch pages.
...
For a fuller walkthrough, see [docs/usage-and-development.md](docs/usage-and-development.md).
...
3. Pre-filters all candidates against the keyword blocklist.
4. Sends remaining candidates to Gemini in batches for AI classification.
5. Validates subtitle eligibility for approved Bilibili videos.
...
8. Pre-translates eligible Chinese-only Bilibili subtitle tracks and caches the approved feed for fast reloading.

### Bilibili subtitle policy

Bilibili videos must be usable in the focused watch page. Videos with downloadable English or bilingual subtitles can pass. Chinese-only videos can pass only when the current Chinese subtitle track is 10 minutes or shorter and can be translated completely. Longer Chinese-only videos are filtered out instead of being sent through a long Gemini translation job.

The translation cache stores source subtitle metadata and a fingerprint. If the current Bilibili subtitle file no longer matches the cached translation, or if the cached translation is partial, the cache is invalidated and the video is withheld unless translation can be regenerated.

Music videos and live performances are a special case: Gemini may label them `music-no-subtitles`, which allows them through without subtitles. Spoken content such as music analysis, interviews, lectures, podcasts, documentaries, and commentary still needs subtitles.
...
- **subtitle-align.js** -- aligns the Chinese overlay line to the active English subtitle cue and merges short Chinese AI subtitle chunks that belong to the same English sentence.
...
The E2E helper checks `CHROMIUM_DEBUG_URL` first, then local DevTools ports such as `9222` and `9223`. Do not skip Chromium E2E tests because the app server or DevTools endpoint is missing; let the helper start the local app and browser where possible.
